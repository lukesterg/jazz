...
async function main() {
  ...
  const bobRecord = { name: 'Bob', age: 5 };
  await database.employees.save(bobRecord);
  await database.contacts.save({ employee: bobRecord, phone: '0411521369' });
  await database.contacts.save({ employee: bobRecord, phone: '0432130558' });

  const aliceRecord = { name: 'Alice', age: 10 };
  await database.employees.save(aliceRecord);
  await database.contacts.save({ employee: aliceRecord, phone: '0411456789' });
